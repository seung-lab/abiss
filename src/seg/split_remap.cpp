...
void split_remap(const remap_t<T> & remap_data, size_t ac_offset, const std::string & tag)
{
    ...
    SlicedOutput<std::pair<T, T>, T> remap_output(str(boost::format("remap/done_%1%.data") % tag));
    SlicedOutput<std::pair<T, size_t>, T> size_output(str(boost::format("remap/size_%1%.data") % tag));
